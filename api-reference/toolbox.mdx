...
- **Tool selection methods**: [`pick()`](#pickctx-options), [`pickAndRun()`](#pickandrunctl-options)
- **Utility methods**: [`assertExhaustive()`](#assertexhaustiveresult), [`getTools()`](#gettools)
...
```typescript
const result = await toolbox.pickAndRun(ctx, { prompt: "Do something" });
